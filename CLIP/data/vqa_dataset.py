        ...
        if split=='train':
            ...
            for f in train_files:
                if self.client is not None:
                    self.annotation += json.loads(client.get(os.path.join('s3://sdcBucket/TokenCompression', ann_root,'%s.json'%f)))
                else:
                    download_url(urls[f],ann_root)
                    self.annotation += json.load(open(os.path.join(ann_root,'%s.json'%f),'r'))
        else:
            if self.client is not None:
                self.annotation = json.loads(client.get(os.path.join('s3://sdcBucket/TokenCompression', ann_root, 'vqa_test.json')))    
                self.answer_list = json.loads(client.get(os.path.join('s3://sdcBucket/TokenCompression', ann_root, 'answer_list.json')))    
            else:
