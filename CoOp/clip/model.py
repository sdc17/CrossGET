...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, token_size: torch.Tensor, token_id: torch.Tensor):
    ...
    def forward(self, x: torch.Tensor, token_id: torch.Tensor = None):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.reduce == 'ours' and self.training:
            x_cross = F.layer_norm(torch.cat(self.transformer.cross_token), \
                self.ln_pre.normalized_shape, self.ln_pre.weight.detach().half(), self.ln_pre.bias.detach().half(), self.ln_pre.eps)
...
def convert_weights(model: nn.Module):
    ...
    if model.reduce == 'ours':
        model.visual.transformer.cross_image.data = model.visual.transformer.cross_image.data.half()
        model.transformer.cross_text.data = model.transformer.cross_text.data.half()


def build_model(state_dict: dict, evaluate: bool, reduce: str, r: dict):
